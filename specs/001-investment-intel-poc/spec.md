...
- **FR-029**: When a signal alert fires, the system MUST attempt to generate a short LLM
  ...
  and append the resulting explanation to the alert's Telegram notification message. The
  explanation is generated by calling the ai-service `POST /explain/{asset}` endpoint directly
  from the Go alert dispatcher (not via Agent Gateway) to minimise latency on the time-critical
  alert delivery path.
