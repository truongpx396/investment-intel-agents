...
- [ ] T076 [P] Verify all API endpoints respond < 200 ms p95 under 50-user simulated load using `k6` (or `hey`); **MUST also assert**: (a) signal evaluation loop peak memory < 256 MB (per NFR-PERF-003) via Go runtime `MemStats` or container `docker stats` during load test; (b) GoClaw digest pipeline completes within 5 min for 50 users × 10 projects (per NFR-PERF-004) — measured via GoClaw OTLP traces and Prometheus `goclaw_llm_call_duration_seconds` + `goclaw_message_send_duration_seconds` metrics, NOT Go pprof (GoClaw is a third-party binary); GoClaw container memory < 512 MB measured via `docker stats` or cAdvisor; add results to `specs/001-investment-intel-poc/research.md`
