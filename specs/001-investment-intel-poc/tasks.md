...
- [ ] T001 Initialise monorepo directory structure per `plan.md` (`backend/`, `ai-service/`, `agent-gateway/`, `frontend/`, `infra/`, `migrations/`)
...
- [ ] T022 [P] [US1] Implement strategy service in `backend/internal/strategies/service.go`: CRUD, status toggle, signal rule validation (cyclomatic complexity ≤ 10 per function); strategy creation MUST validate `app_projects.is_signal_asset = TRUE` for the selected asset and reject with a clear error if `FALSE` (per FR-002)
...
**Goal**: User manages a watchlist; Agent Gateway digest agent (GoClaw default) runs daily at 08:30 UTC, fetches news via CryptoPanic, summarises with Claude (claude-3-5-haiku), sends one Telegram message per user with per-project sections.
...
- [ ] T057a [P] [US3] Define and implement service-to-service auth for Agent Gateway → backend API calls: `AGENT_GATEWAY_INTERNAL_TOKEN` env var; add `/internal/` route prefix in Traefik config validated by a static-bearer middleware (bypasses Supabase JWT); document token scheme in `contracts/rest-api.md`; unit test the middleware — **note (Apr 2026)**: additionally configure **per-agent grants** in the Agent Gateway (e.g., GoClaw) to scope the digest agent's access to only the `/internal/watchlist` and `/internal/users` endpoints; per-agent grants provide a finer-grained permission layer on top of the static bearer token; if the provider type is changed, the Agent Gateway will automatically re-validate for SSRF
...
- [ ] T072 [P] Import Grafana dashboards: Go API latency (p50/p95/p99), NATS throughput, Agent Gateway LLM call latency + cost, signal evaluation cycle time
...
|-------|---------|-------|---------------|
| 1 – Setup | — | T001–T010c, T084–T086 (incl. T001a, T009a) | Blocks everything |
...
| 7 – Watchlist + Digest | US3 (P3) | T048–T059 (incl. T056a) | Agent Gateway + AI layer |
