...
- [ ] T006 [P] Set up `docker-compose.yml` for local dev: Postgres 18 + pgvector, Redis 7, NATS JetStream, Traefik, GoClaw (`latest` image — web UI embedded, dashboard at `http://localhost:18790`), backend, ai-service, frontend — **no separate `goclaw-web` container** (Apr 2026: web UI is compiled into the Go binary)
