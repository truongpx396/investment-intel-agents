...
- [ ] T039a [P] [US2] Implement CryptoCompare `histominute` adapter in `pkg/marketdata/cryptocompare.go` for intraday OHLCV data; unit tests with mocked HTTP; the adapter fetches the rolling 14-candle OHLCV slice and forwards it to `ai-service GET /indicators/{asset}` — Go does NOT compute RSI directly; CoinGecko adapter handles spot price + % change signals only
- [ ] T040 [P] [US2] Implement technical indicators endpoint in `ai-service/src/indicators/`: `rsi.py` (14-period RSI using `pandas-ta` from OHLCV slice), `price_stats.py` (24 h open/high/low/close/pct_change), `cache.py` (Redis key `indicators:{asset}` TTL = 30 s), `router.py` (`POST /indicators/{asset}` — accepts OHLCV slice, returns `{ rsi, price_stats }`, checks cache first); Go signal evaluator calls this endpoint instead of computing RSI itself; unit tests covering overbought (>70), oversold (<30), neutral, and cache-hit path
- [ ] T040a [P] [US2] Update Go signal evaluator (`internal/signals/evaluator.go`) for RSI signal type: after fetching the 14-candle OHLCV slice via CryptoCompare adapter, `POST` the slice to `ai-service/indicators/{asset}`; parse the `rsi` field from the response; evaluate the overbought/oversold threshold against it; cache the response in Redis at `indicators:{asset}` with TTL = 25 s to avoid redundant ai-service calls within the same 30 s tick window; unit tests MUST cover: RSI signal path using mocked `httpx` response from ai-service, cache-hit skips HTTP call, ai-service timeout falls back to skipping the tick (log + continue, do NOT fire alert on error)
...
- [ ] T053c [P] [US3] Implement curated project registry in `ai-service/src/projects/registry.py`: static mapping of `project_slug → { display_name, symbol, coingecko_id }`; expose `GET /projects` endpoint returning the full list; used by React watchlist page and GoClaw skill to resolve slugs; unit test the registry mapping completeness (all ~20 POC projects present)
