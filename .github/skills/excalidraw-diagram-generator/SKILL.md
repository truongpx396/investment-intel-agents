...
## Critical Rules (Prevent "Failed to load Document" Errors)

> ⚠️ Violations of these rules cause the VS Code Excalidraw extension to show **"Failed to load Document: Error: Unable to load initial"** and the file will not open.

### 1. NEVER use `containerId` / `boundElements` binding for text labels

The most common mistake is placing text inside a rectangle using `containerId`. This pattern **crashes the VS Code Excalidraw extension**.

```json
// ❌ WRONG — causes "Failed to load Document" error
{ "id": "box1", "type": "rectangle", "boundElements": [{"id": "txt1", "type": "text"}] }
{ "id": "txt1", "type": "text", "containerId": "box1" }

// ✅ CORRECT — free-positioned text overlaid on rectangle
{ "id": "box1", "type": "rectangle", "boundElements": [] }
{ "id": "txt1", "type": "text", "containerId": null, "x": <same-or-near box x>, "y": <same-or-near box y> }
```

**Rule**: Every element must have `"boundElements": []` (empty array, never `null`, never with entries). Text labels must have `"containerId": null` and be **free-positioned** over/near the shape they label.

### 2. `index` values MUST be in strict lexicographic order

Excalidraw uses fractional indexing. Every element's `index` must be strictly greater than the previous element's `index` in lexicographic order.

```json
// ❌ WRONG — c3n > c3 lexicographically, so c3 is out of order
["c2", "c3n", "c3", "c4"]
...
**Recommended**: Use zero-padded sequential indices: `a00`, `a01`, `a02`, ..., `a68`. This guarantees correct order regardless of how many elements are inserted.

### 3. All text elements MUST have `autoResize` and `lineHeight`

```json
// ✅ Required fields on every text element
{
  "type": "text",
  "containerId": null,
  "autoResize": true,
...
### 4. All line/arrow elements MUST have these fields

```json
// ✅ Required fields on every line and arrow element
{
  "type": "arrow",
  "points": [[0, 0], [dx, 0]],
  "startBinding": null,
...
### 5. Use the correct `source` URL for VS Code extension

```json
// ✅ VS Code Excalidraw extension
"source": "https://marketplace.visualstudio.com/items?itemName=pomdtr.excalidraw-editor"

// ⚠️ Only use this if targeting excalidraw.com web app
"source": "https://excalidraw.com"
```

### 6. Use Python script generation for complex diagrams

For diagrams with many elements, generate via a Python script with helper functions to avoid manual JSON errors:

```python
import json, random
random.seed(42)  # reproducible seeds

def text(id, x, y, w, h, txt, idx, **kw):
    return {
        ...
        "seed": random.randint(1000, 999999), "version": 1,
        "versionNonce": random.randint(1000, 999999),
        "isDeleted": False, "boundElements": [],
        "updated": 1706659200000, "link": None, "locked": False,
        "text": txt, "fontSize": kw.get("fontSize", 16),
        "fontFamily": 5, "textAlign": kw.get("textAlign", "center"),
        "verticalAlign": "top", "containerId": None,
        "originalText": txt, "autoResize": True, "lineHeight": 1.25
    }

def rect(id, x, y, w, h, idx, **kw):
    return {
        ...
        "roughness": kw.get("roughness", 1), "opacity": kw.get("opacity", 100),
...
def arrow(id, x, y, dx, idx, **kw):
    return {
        "id": id, "type": "arrow", "x": x, "y": y,
        "width": abs(dx), "height": 0, "angle": 0,
...
# Assign indices at the END after all elements are assembled
elements = []
# ... build elements ...
for i, el in enumerate(elements):
    el["index"] = f"a{i:02d}"  # a00, a01, a02, ...

# Verify before writing
indices = [e["index"] for e in elements]
assert indices == sorted(indices), "Indices not sorted!"
...
| Issue | Solution |
|-------|----------|
| "Failed to load Document" error | Check: (1) no `containerId` bindings, (2) `index` values are sorted lexicographically, (3) all required fields present on each element type |
| Text not visible inside boxes | Use free-positioned text (NO `containerId`). Position text element at same x/y as rectangle |
...
- [ ] **`index` values are strictly lexicographically sorted** (use `a00`, `a01`, ... assigned at the end)
- [ ] **No element has `containerId` set** (must be `null`)
- [ ] **All elements have `boundElements: []`** (empty array, never `null`)
- [ ] **All text elements have `autoResize: true` and `lineHeight: 1.25`**
- [ ] **All arrow/line elements have `startBinding: null`, `endBinding: null`, `lastCommittedPoint: null`**
- [ ] `source` is the VS Code marketplace URL (not excalidraw.com)
- [ ] `appState` includes `gridSize`, `gridStep`, `gridModeEnabled`
...
```python
import json
with open('your-diagram.excalidraw') as f:
    doc = json.load(f)

indices = [e['index'] for e in doc['elements']]
assert indices == sorted(indices), f"Indices out of order!"
assert len(indices) == len(set(indices)), "Duplicate indices!"
for e in doc['elements']:
    assert e.get('boundElements') == [], f"{e['id']} has non-empty boundElements"
    if e['type'] == 'text':
        assert e.get('containerId') is None, f"{e['id']} has containerId set"
        assert 'autoResize' in e, f"{e['id']} missing autoResize"
        assert 'lineHeight' in e, f"{e['id']} missing lineHeight"
    if e['type'] in ('arrow', 'line'):
        assert 'lastCommittedPoint' in e, f"{e['id']} missing lastCommittedPoint"
print(f"✓ Valid: {len(doc['elements'])} elements")
```
